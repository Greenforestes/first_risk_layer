...
class DynamicActorManager(Node):
    def __init__(self):
        ...
        # synchronized trial start
        self.declare_parameter('trial_start_topic', '/trial/start')
        self.trial_start_topic = self.get_parameter(
            'trial_start_topic'
        ...
        self.trial_start_sub = self.create_subscription(
            Empty,
            self.trial_start_topic,
            self.trial_start_callback,
            10
        )
        # Timer
        ...
        self.get_logger().info(
            f'waiting for synchronized trial start on topic: {self.trial_start_topic}'
        )

    def trial_start_callback(self, _msg: Empty):
        if self.trial_started:
            return

        self.trial_started = True
        self.last_time = self.get_clock().now()
        self.get_logger().info('=== Trial start received, actor motion enabled ===')
    ...
    def timer_callback(self):
        ...
        # )
        # 未收到 /trial/start 之前保持静止；收到后才真正开始移动
        if self.trial_started:
            self.progress, self.direction = self.advance_progress(
                self.progress, self.direction, self.speed * dt
            )
